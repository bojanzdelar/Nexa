import type { CategoryResponse, Show } from "~/types";

export const getTrendingShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/trending");
};

export const getTopRatedShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/top-rated");
};

export const getAiringTodayShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/latest");
};

export const getDramaShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/genres/18");
};

export const getSciFiFantasyShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/genres/10765");
};

export const getComedyShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/genres/35");
};

export const getCrimeShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/genres/80");
};

export const getMysteryShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/genres/9648");
};

export const getActionAndAdventureShows = (ssr: boolean = true) => {
  return useCatalogApi(ssr)<CategoryResponse<Show>>("/tv/genres/10759");
};

export const getShowDetails = (id: number, ssr: boolean = true) => {
  return useCatalogApi(ssr)<Show>(`/tv/${id}`);
};
