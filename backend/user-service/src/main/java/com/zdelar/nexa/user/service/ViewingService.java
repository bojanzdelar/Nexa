...
public class ViewingService {
  ...
  public ViewingService(UserRepository userRepository, UserMapper userMapper) {
    this.userRepository = userRepository;
    this.userMapper = userMapper;
  }

  public List<ContinueWatchingResponse> getContinueWatching(String userId, int limit) {
    return userRepository.findAllByPkAndSkPrefix(userPk(userId), VIEWING_PREFIX).stream()
  ...
  public ProgressResponse getMovieProgress(String userId, Long movieId) {
    return userRepository
        .findByKey(userPk(userId), viewingMovieSk(movieId))
        .map(userMapper::toProgressResponse)
        .orElse(new ProgressResponse(0));
  }
  ...
  public void updateEpisodeProgress(String userId, EpisodeProgressRequest request) {
    ...
    if (completed) {
      userRepository.delete(
          userPk(userId), viewingEpisodeSk(request.tvId(), request.season(), request.episode()));
      return;
    }

    userRepository.put(userMapper.toEpisodeViewingItem(userId, request));
  }

  public void updateMovieProgress(String userId, MovieProgressRequest request) {
    boolean completed = isCompleted(request.progressSeconds(), request.durationSeconds());
    if (completed) {
      userRepository.delete(userPk(userId), viewingMovieSk(request.movieId()));
      return;
    }

    userRepository.put(userMapper.toMovieViewingItem(userId, request));
  }

  public void clearTvProgress(String userId, Long tvId) {
    userRepository
        .findAllByPkAndSkPrefix(userPk(userId), viewingTvSk(tvId))
  ...
  public void clearEpisodeProgress(String userId, Long tvId, int season, int episode) {
    userRepository.delete(userPk(userId), viewingEpisodeSk(tvId, season, episode));
  }

  public void clearMovieProgress(String userId, Long movieId) {
    userRepository.delete(userPk(userId), viewingMovieSk(movieId));
  }
